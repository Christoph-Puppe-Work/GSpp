...
> **Note on history:** The 1:1 Anforderung→Control mapping stage (`stage_matching`) was removed; an ED2023 profile now includes **all** controls of the matched Zielobjektkategorie and is enriched per Baustein. The enhancement stage is `stage_ED23_profiles_enhanced.py` (formerly `stage_profiles_enhanced.py`, originally `stage_component.py`); it enriches profiles with OSCAL `alter` blocks driven by best practices and the Baustein description.
...
**Impact:** For classification and structured JSON output, this increases the risk of hallucinations, inconsistent results across runs, and schema validation failures.
**Recommendation:** Reduce the `API_TEMPERATURE` significantly (e.g., 0.1 - 0.3) for the classification fields, or split classification from prose generation.
...
**Description:** The enhanced profile stage relies heavily on AI to generate the prose (Statement, Guidance, Assessment) for maturity levels 1, 2, 4, and 5, and now does so for *every* control of the Zielobjektkategorie.
...
**Location:** `src/assets/json/prompt_config.json` (Rules A and B), `src/pipeline/stage_ED23_profiles_enhanced.py`
...
**Recommendation:** Deterministically inject the known Level 3 prose (the G++ control prose is already available via `gpp_controls_lookup`) instead of asking the model to copy it. The AI prompt and schema should then only request levels 1, 2, 4, and 5.
...
### 3.4. Dead Google Cloud Storage Configuration
...
### 3.5. No Timeout or Offline Fallback on Remote Data Fetch
...
**Description:** Helpers that were only used by the removed `stage_matching` (e.g. `parse_zielobjekte_hierarchy`, `parse_bsi_2023_controls`) may now be unused.
**Recommendation:** Confirm and remove any now-dead parsing helpers.
