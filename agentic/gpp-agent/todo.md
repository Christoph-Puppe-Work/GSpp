...
- [x] **Classifier Jump-Back Bug**: The `classifier` agent calls `route_to_phase`, but the workflow jumps back to the `chat` loop instead of routing to the target phase. Root cause: the tool wrote directly to `tool_context.session.state`, bypassing ADK's tracked tool state delta; after that fix, ADK still fed the FunctionResponse back to the classifier for summarization, causing repeated tool calls before the workflow reached `classify_router`; after that fix, the classifier still ran in unsupported graph-workflow `chat` mode and ended the agent before the router. Fixed by writing through `tool_context.state`, setting `tool_context.actions.skip_summarization = True`, switching the classifier to `single_turn`, and ending ambiguous no-route turns instead of looping to the classifier within the same invocation.
- [x] **Phase 1 Governance Backend Tool Loop / Missing MCP Session Context** (observed on 2026-05-11, approx. 21:42:16-21:43:28 Europe/Berlin): The classifier routing bug appears fixed: the workflow reaches `route="govern"` and enters Phase 1. The remaining bug starts after Phase 1 begins using the backend MCP. Server logs show repeated `CallToolRequest` cycles, repeated Vertex LLM calls, and repeated backend messages:
  `ERROR:GppContextMCP.utils:Could not find session info in context` followed by
  `WARNING:GppContextMCP.utils:Using local development IV fallback 'local-dev' because MCP session context is unavailable.`
  This means the backend MCP tool receives no usable `ctx.request_context.session.user_id` in the expected `{caller}::iv::{iv_id}` format. The explicit local fallback keeps the local run from failing immediately, but it is not proof that the correct IV/session is propagated.

  Current working hypothesis: this is a context/auth propagation problem between the local ADK MCP client / authenticated tool path and `GS_backend_MCP`, not a Phase-1 tool-filter problem. The loop may be the Phase-1 LLM repeatedly trying backend tools after tool results are empty, failing, or tenant-context ambiguous. The fix should preserve Phase 1's access to the backend MCP tools long term; do not "solve" this by shrinking Phase 1 to one backend tool. Also do not cap `max_steps` as a general solution: SSP creation and governance review can legitimately require long, multi-step, human-in-the-loop work.

  Next investigation items:
  - Capture the ADK event trace around the first Phase-1 backend tool call, including tool name, arguments, FunctionResponse content, and state deltas.
  - Inspect how the local MCP client sets authenticated/session context for `McpToolset` calls, especially under the experimental `BASE_AUTHENTICATED_TOOL` path shown in the logs.
  - Decide whether local development should inject a deterministic test user/IV into the MCP session context instead of relying on backend-side fallback.
  - Keep the backend fallback logging change, but avoid treating fallback as the real tenant-isolation fix.
  - After context propagation is fixed, rerun the "create/start new SSP" scenario and confirm Phase 1 either returns a valid `GovernanceFindings` object or reaches the HITL gate exactly once, without repeated backend tool retries.
